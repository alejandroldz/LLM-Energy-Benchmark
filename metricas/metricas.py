...
class Metricas:
    ...
    def guardar_csv(self, config: ConfigExperimento, path: str):    
        df = pd.DataFrame({
            ...
            "Attention Implementation": [config.attention_implementation],
            "Speculative Decoding": [config.speculative_decoding["method"] if config.speculative_decoding else None]
        })
